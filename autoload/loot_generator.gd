...
func roll_item(floor_num: int, rarity: int) -> ItemData:
...
## suffixes add character stats.
func _roll_affixes(item: ItemData, floor_num: int) -> void:
	...
	var magnitude: int = item.rarity + rng.randi_range(0, 1) + floori((floor_num - 1) / 2.0)
	...
	for i in mini(count, suffix_pool.size()):
		...
		var amount: int = 1 + rng.randi_range(0, 1) + floori(item.rarity / 2.0)
...
func open_box(tier: int, floor_num: int) -> Array:
	...
	for i in count:
		if rng.randf() < 0.25:
			...
			items.append(roll_item(floor_num, roll_rarity(tier, floor_num)))
	return items
